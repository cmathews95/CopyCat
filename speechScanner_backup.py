# Read through cleanly formatted speech
# and create a probability matrix of 
# the probablity of two words being next
# to each other.
...
# Once you have dictionary, create a 2d Array
# with number of elements in dictionary
index = SPEECH_DICT.keys()
#print(index)
print(len(index))
print("-----LENGTH-----")
#create 2d array = probMatrix
#probMatrix = [len(index)][len(index)]
lastWord = "notaword"
...
try:
    for line in f:
		...
		word = word.replace("?", "")
    		...
    		# Add Word to Dict. or increment count
		if word in SPEECH_DICT:
			#Set word1, else set word2 & increment probMatrix[word1][word2]
			if (lastWord == "notaword"):
				lastWord = word
			else:
				probMatrix[lastword][word]+=1
except IOError as e:
	print "I/O Error in Creating Probability Matrix({0}): {1}".format(e.errno, e.strerror)
